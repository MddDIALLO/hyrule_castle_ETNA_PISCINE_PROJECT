...
import { gameStarting, difficultyChoice, updateEnemyStats} from './mods/basic_game_customization';
...
let playing: boolean = gameStarting();
...
if(difficulty.length > 0 && (difficulty === 'Difficult' || difficulty === 'Insane')) {
    enemy = updateEnemyStats(enemy, difficulty);
    bosse = updateEnemyStats(bosse, difficulty);
}
